...
Route::prefix(FrontDeskUser::DASHBOARD_ROUTE_PREFIX)->name(FrontDeskUser::ROUTE_NAME_PREFIX)->group(function () {
  ...
  Route::middleware('auth:' . collect(config('auth.guards'))->except(['web','api'])->keys()->implode(','))->group(function () {
    ...
    Route::get('create', [FrontDeskUserController::class, 'create'])->name('create_users')->defaults('menu', __e('Manage Front Desk Users', 'viewAny,' . FrontDeskUser::class, 'users', 2, false, 'Manage Users', 1));

    Route::get('destroy', [FrontDeskUserController::class, 'destroy'])->name('remove_users')->defaults('menu', __e('Manage Front Desk Users', 'viewAny,' . FrontDeskUser::class, 'users', 2, false, 'Manage Users', 1));
